...
class LocalEmbeddingIndex:
    ...
    def build_from_clean(self, clean_path: Path | None = None) -> "LocalEmbeddingIndex":
        clean_file = clean_path or self.settings.paths.clean_json
        if not clean_file.exists():
            clean_file = self.settings.paths.clean_csv
        df = pd.read_json(clean_file) if str(clean_file).endswith(".json") else pd.read_csv(clean_file)
    ...
    def semantic_search(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        """Alias for search method to support semantic query requests."""
